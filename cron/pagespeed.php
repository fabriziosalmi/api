...
$stmt = $conn->prepare("SELECT url_id FROM monitor_link WHERE monitor_id = 4 AND status = 1;");
$stmt->execute();
$array = [];
...
foreach ($array as $url_id_to_monitor) {
    ...
    // monitor
    $api = "https://charts.rivoluzioneinformatica.org/api/pagespeed.php?url=";
    file_get_contents($api.$url);
}

$conn->close();
print_r("<p>monitor ok</p>");
